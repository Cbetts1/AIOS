...
class RunCommand:
    ...
    def execute(self, args, eal) -> int:
        ...
        if not os.path.isfile(filepath):
            print(f"[run] File not found: {filepath}")
            return 1
        ...
        if entry is None:
            ...
            print(
                f"[run] No runtime found for extension '{ext}'.\n"
                "      Make the file executable or add a shebang line."
            )
            return 1

        runtime_bin, extra_flags = entry
        if not shutil.which(runtime_bin):
            print(f"[run] Runtime '{runtime_bin}' not found in PATH.")
            return 1
